"""Thin wrapper around py-clob-client: the only module allowed to touch the
authenticated CLOB API. Field names below (OrderArgs, OrderBookSummary,
AssetType, etc.) are taken verbatim from py-clob-client's clob_types.py —
confirm against your installed version in the smoke test before going live.
"""
...
def get_min_order_size(token_id: str) -> float:
    ...
    return float(books[0].min_order_size) if books and books[0].min_order_size else 0.0
...
def place_limit_sell(token_id: str, limit_price: float, size_shares: float) -> PlacedOrder:
    client = get_client()
    order_args = OrderArgs(token_id=token_id, price=limit_price, size=round(size_shares, 2), side=SELL)
    signed_order = client.create_order(order_args)
    resp = client.post_order(signed_order, OrderType.GTC)
    return PlacedOrder(order_id=resp.get("orderID", ""), raw_response=resp)
